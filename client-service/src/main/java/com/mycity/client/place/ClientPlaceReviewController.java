...
//	public Mono<String> addPlaceReview(@RequestBody ReviewDTO dto,
...
//		return webClientBuilder.build().post().uri("lb://" + API_GATEWAY_SERVICE_NAME + REVIEW_ADDING_PATH)
//				.header(HttpHeaders.AUTHORIZATION, "Bearer " + token).body(Mono.just(dto), ReviewDTO.class).retrieve()
...
//	public Mono<String> updatePlaceReview(@PathVariable Long reviewId, @RequestBody ReviewDTO dto,
//			@RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie) {
//		System.out.println("ClientPlaceReviewController.updatePlaceReview()");
//		// Extract Token From Cookie
//		String token = extractor.extractTokenFromCookie(cookie);
//		return webClientBuilder.build().put().uri("lb://" + API_GATEWAY_SERVICE_NAME + REVIEW_UPDATING_PATH, reviewId)
//				.header(HttpHeaders.AUTHORIZATION, "Bearer " + token).body(Mono.just(dto), ReviewDTO.class).retrieve()
//				.onStatus(HttpStatusCode::isError, clientResponse -> clientResponse.bodyToMono(String.class)
//						.flatMap(errorBody -> Mono.error(new RuntimeException(
//								"Failed to Update Review: " + clientResponse.statusCode() + " - " + errorBody))))
//				.bodyToMono(String.class).onErrorResume(e -> Mono.just("Failed to Update Review: " + e.getMessage()));
//	}
//
//	@GetMapping("/getreview/{placeId}")
//	public Mono<List<ReviewSummaryDTO>> getReviewsByPlaceId(@PathVariable Long placeId,
//			@RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie) {
...
//		String token = extractor.extractTokenFromCookie(cookie);
//
//		return webClientBuilder.build().get().uri("lb://" + API_GATEWAY_SERVICE_NAME + REVIEWS_GETTING_PATH, placeId)
...
//	public Mono<String> deleteReviewsByPlaceId(@PathVariable Long reviewId,
//			@RequestHeader(value = HttpHeaders.COOKIE, required = false) String cookie) {
